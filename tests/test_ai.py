...
def mock_gemini_response():
    with patch("app.services.ai.GEMINI_API_KEY", "mock_api_key"), patch(
        "app.services.ai.genai.GenerativeModel"
    ) as mock_model:
...
def test_summarize_note(client, mock_gemini_response):
    ...
    create_response = client.post(
        ...
        json={
            "title": "Test Note",
            "content": (
                "This is a test note content that needs to be summarized."
            ),
        },
    )
