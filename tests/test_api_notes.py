...
def test_get_note(client):
    ...
    create_response = client.post("/notes/", json={"title": "Test", "content": "Test content."})
...
def test_update_note(client):
    ...
    create_response = client.post("/notes/", json={"title": "Original", "content": "Content"})
    note_id = create_response.json()["id"]

    response = client.put(f"/notes/{note_id}", json={"title": "Updated", "content": "Updated content"})
    assert response.status_code == 200
