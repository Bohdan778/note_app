...
from app.services.ai import summarize_text, summarize_text_async, list_available_models
...
def mock_available_models():
    with patch("app.services.ai.genai.list_models") as mock_list_models:
        mock_list_models.return_value = [
            MagicMock(name="gemini-1.0-pro"), MagicMock(name="gemini-pro"), MagicMock(name="gemini-1.5-pro")
        ]
        yield
...
def test_summarize_text_with_error(mock_gemini_api_key, mock_available_models):
    """Перевіряє, чи правильно обробляється помилка при самаризації через API"""
    with patch("app.services.ai.genai.GenerativeModel") as mock_model:
        mock_instance = MagicMock()
...
@pytest.mark.asyncio
async def test_summarize_text_async(mock_gemini_api_key, mock_available_models):
...
@pytest.mark.asyncio
async def test_summarize_text_async_with_error(mock_gemini_api_key, mock_available_models):
    """Перевіряє, чи правильно обробляється помилка при асинхронній самаризації"""
    with patch("app.services.ai.genai.GenerativeModel") as mock_model:
        mock_instance = MagicMock()
        mock_instance.generate_content.side_effect = Exception("API error")
        mock_model.return_value = mock_instance
...
def test_list_available_models(mock_gemini_api_key):
    ...
    with patch("app.services.ai.genai.list_models") as mock_list_models:
        ...
        assert any("gemini" in model.lower() for model in models), f"Models found: {models}"
